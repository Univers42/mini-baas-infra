...
[[ -f "$SCRIPT_DIR/test-ui.sh" ]] && source "$SCRIPT_DIR/test-ui.sh" 2>/dev/null || {
  ...
  ui_summary() {
    echo ""
    echo -e "${BOLD}Summary:${NC} Passed=$1 Failed=$2"
    [[ "$2" -eq 0 ]] && echo -e "${GREEN}$3${NC}" || echo -e "${RED}$4${NC}"
    [[ "$2" -eq 0 ]] && exit 0 || exit 1
  }
}
...
HTTP=$($CURL -o "$TMPDIR/health.json" -w '%{http_code}' \
...
HTTP=$($CURL -o "$TMPDIR/signup.json" -w '%{http_code}' \
...
HTTP=$($CURL -o "$TMPDIR/login.json" -w '%{http_code}' \
...
HTTP=$($CURL -o "$TMPDIR/protected.json" -w '%{http_code}' \
  "$BASE_URL/rest/v1/" \
...
HTTP=$($CURL -o "$TMPDIR/unauth.json" -w '%{http_code}' \
...
HTTP=$($CURL -o "$TMPDIR/forged.json" -w '%{http_code}' \
  "$BASE_URL/rest/v1/" \
...
if [[ -n "$REFRESH_TOKEN" && "$REFRESH_TOKEN" != "null" ]]; then
  HTTP=$($CURL -o "$TMPDIR/refresh.json" -w '%{http_code}' \
  ...
  HTTP=$($CURL -o "$TMPDIR/new_access_test.json" -w '%{http_code}' \
    "$BASE_URL/rest/v1/" \
...
HTTP=$($CURL -o "$TMPDIR/user.json" -w '%{http_code}' \
...
HTTP=$($CURL -o "$TMPDIR/headers_test.json" -w '%{http_code}' \
  "$BASE_URL/rest/v1/" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "$HDR_APIKEY" 2>/dev/null)
...
# ═══════════════════════════════════════════════════════════════════
ui_step "11. Logout"
HTTP=$($CURL -o "$TMPDIR/logout.json" -w '%{http_code}' \
...
HTTP=$($CURL -o "$TMPDIR/wrong_pass.json" -w '%{http_code}' \
  -X POST "$BASE_URL/auth/v1/token?grant_type=password" \
  -H "$CT_JSON" -H "$HDR_APIKEY" \
